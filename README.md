...
## 国外网站配置规则
> 国外时间会解析失败，请及时联系我，更改时间解析！

```python
# 根据网站的实际规则填写
# 注意：填写网站实际名称，把繁体转换为简体
"ir_mediasource": "香港工业总会",  # 文章媒体来源

# 地区根据 exls 表格填写，注意填写中文
"ir_librariytype": '香港',  # 地区

# 把监控区域更改为1
"ir_area": 1,  # 监控区域  1 == 国外 2 == 国内
```

### 时间注意事项
> 国外时间格式不一样 请特别留意下面的格式，好更改模板，避免时间解析错误   
> 01/03/2019 时间在海外往回网站有两种时间解释：   
> 1. 01/03/2019 == 1月/3号/2019年  这是框架得默认解析时间   
> ----------------------------------------------------    
> 2. 01/03/2019 == 1号/3月/2019年  这个时间跟上面时间的冲突    
> 需要在模板配置 "parse_date_mode":"parse_english_day_mon_year" 解决时间解析冲突造成时间解析错误
...
"pub_time": {
    ...
    "parse_date_mode": "parse_english_day_mon_year",
},
```
